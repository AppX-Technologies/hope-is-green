...
export function BoardMemeberSettings() {
  return (
            ...
            {BoardMemberFormField.map((group) => (
              <div key={group.groupName} className="flex flex-wrap mb-4">
                ...
                {group?.question?.map(({ label, name, type }) => {
                  return (
                    <div key={name} className="w-full md:w-1/3 px-2"> {/* Adjusted width to 1/3 for three items in a row */}
                      ...
                      <div className="relative">
                        ...
                        {type === "selectedPosition" && (
                          <Select
                            options={["Chairman", "Board administrator", "Board"]}
                            onChange={(e) =>
